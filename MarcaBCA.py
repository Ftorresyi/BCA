...
def BuscaOMsApoiadas():
    ...
    while j < ltotal:
        ...
        while i < len(OMs):
            ...
            if OM_procurada != []:
                print('A OM Apoiada: ', set(OM_procurada), 'foi encontrada ', len(OM_procurada), ' vez(es)',
                      'na página', numPage + 1)
                BuscaResult = f'''
        ...
        for i in OMs:  # loop busca OM por OM da lista OMs em cada pagina do BcadoDia
            ...
            if text_instances != []:  # Se a lista de OMs nao for vazia
                for j in text_instances:  # Percorre a lista de palavras achadas uma a uma
                    ...
                    print('Foi encontrada a palavra: ',
                          highlight)  # imprime a palavra marcada que eh um objeto da classe Fitz
